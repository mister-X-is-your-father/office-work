# backend-patch — ネイティブ実績時間トラッキング

TaskStation を fork して **実績時間（worklog）と見積りを DB に第一級で持たせる**ためのパッチ。
...
> 注: これは設計に忠実なスキャフォルド。実ビルドでは TaskStation の errors 規約に沿った専用エラー型の追加、
