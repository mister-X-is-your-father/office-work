...
- **TickTick機能パリティ一式**（2026-06-13・機能の洗い出し→TaskStation流にブラッシュアップして移植。全テスト80件グリーン・全20ルート巡回エラー0で確認）:
  ...
  - **集中タイマー（ポモドーロ）**（`views/pomodoro.js`・トップバー🍅）: 集中15/25/45/50分・休憩5/10/15分・一時停止/中断。**終了/中断時に選択タスクの実績(time entries)へ自動記録**（90秒未満は記録しない）→見積りvs実績・予実ガントに直結（TickTickは専用統計どまり＝ここがブラッシュアップ）。状態は localStorage `ts.pomo`（リロード/タブ閉じ継続・不在中の満了は復帰時に記録）。タブタイトルに残り時間。
