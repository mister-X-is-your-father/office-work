...
- **TickTick機能パリティ一式**（2026-06-13・機能の洗い出し→TaskStation流にブラッシュアップして移植。全テスト80件グリーン・全20ルート巡回エラー0で確認）:
  ...
  - **集中タイマー（ポモドーロ）**（`views/pomodoro.js`・トップバー🍅）: 集中**5/10/15/25/45/50/60分**・休憩5/10/15分・一時停止/中断。**カードはヘッダ掴みでドラッグ移動**（位置localStorage永続）・モード切替で高さ固定（`.pm-modebox`）。**表示スキン カタログ8種**（標準/ミニマル/バー/リング/セグメント/特大/コンパクト/ドット＝`renderDisplay`・進捗`progressOf`）＋**色・透明度カスタム**（🎨ピッカー・`ts.pomo.disp`）。実行カードは「表示エリアのみ毎秒更新・操作部/ピッカーは永続」構造でチラつき/入力消失なし。スキン/色はPiPにも反映。**最前面表示=Document PiP は secure context 専用**＝PWA(https://leo…:7011/app/)でのみ動作（平文httpは案内表示）。**終了/中断時に選択タスクの実績(time entries)へ自動記録**（90秒未満は記録しない）→見積りvs実績・予実ガントに直結（TickTickは専用統計どまり＝ここがブラッシュアップ）。状態は localStorage `ts.pomo`（リロード/タブ閉じ継続・不在中の満了は復帰時に記録）。タブタイトルに残り時間。
...
- **一覧の個人別並び順**（2026-06-13・`views/table.js`）: 並び設定（ソート基準＋手動順＋絞り込み）を**見ている本人ごとに localStorage 保存**（`ts.list.view.<uid>`・**DB共有データは不変＝他メンバーの見え方に影響しない＝衝突しない**）。並び順セレクトに**「マイ並び（手動）」**追加→⠿グリップでドラッグ並べ替え（HTML5 DnD・上下半分で挿入位置判定・フィルタ中でも非表示分の順序を保持して全体順を再構築・新規は末尾・欠番除去）。「やる順／PJ順／優先度順／好み」を各自が独立して持てる。※端末ごと＝クロスデバイス同期は将来（user_preferences表が必要）。
