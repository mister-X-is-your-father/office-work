...
(b) `addMoreInfoToTasks(...)` の末尾近く（`addAssigneesToTasks` 等を呼んでいる箇所）に SUM ローダ呼び出しを追加:
```go
	// 実績合計(time_spent)を attach
	spentMap, err := sumTaskTimeSpent(s, taskIDs)
	if err != nil {
		return err
	}
...
```
（`sumTaskTimeSpent` は `task_time_entry.go` に同梱済み。`taskIDs` / `taskMap` は同関数内の既存変数。）
