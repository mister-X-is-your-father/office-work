...
// 分類ルール（画面の凡例にも明記）:
//   重要 = 優先度が「高」以上（priority>=3） / 緊急 = 期日が today+N日以内 or 超過（期日なし=緊急でない）。
...
export async function render(root) {
  ...
  root.innerHTML = `
    ...
    <div class="qd-tools">
      <span class="qd-rule">📐 <b>重要</b> = 優先度が「高」以上 ／ <b>緊急</b> = 期日が
        <select id="qd-days">${dayOpts}</select>
        以内 または超過（期日なし＝緊急でない）</span>
      <select id="qd-who">${whoOpts}</select>
    </div>
    <div class="qd-matrix">
      <div class="qd-corner"></div>
      <div class="qd-ax-x"><span>◀ 緊急（期日が近い）</span><span>緊急でない ▶</span></div>
    ...
    <div class="qd-hint">ドラッグでの移動はタスクを書き換えます: 重要側へ=優先度を高に ／ 緊急側へ=期日を今日に ／ 緊急でない側へ=期日を＋7日（取り消しは移動直後の「元に戻す」）。</div>
