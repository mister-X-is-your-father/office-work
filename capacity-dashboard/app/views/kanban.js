...
export async function render(root) {
  ...
  const colHtml = (b) => {
    const tasks = (b.tasks || []);
    const cards = tasks.map((t) => cardHtml(t, memberIdx, today)).join("");
    return `<div class="kb-col" data-bucket="${b.id}">
      <div class="kb-colh">
        <span class="kb-colt" data-rename="${b.id}" title="クリックで名前変更">${esc(b.title)}</span>
        <span class="kb-cnt">${tasks.length}</span>
        ${tasks.length === 0 ? `<button class="kb-del" data-del="${b.id}" title="空の列を削除">×</button>` : ""}
      </div>
  ...
  root.innerHTML = `
    <style>${css()}</style>
    <h1 class="vtitle">かんばん <small>カードをドラッグで移動 ・ クリックで編集</small></h1>
    <div class="kb-tools">
  ...
  root.querySelectorAll("[data-del]").forEach((b) => {
    b.onclick = async () => { await deleteBucket(_pid, _vid, +b.dataset.del); render(root); };
  });
  ...
  root.querySelectorAll(".kb-card").forEach((card) => {
    card.onclick = () => openTaskForm({ taskId: +card.dataset.id, onSaved: () => { invalidate(); render(_root); } });
...
function cardHtml(t, memberIdx, today) {
  ...
  return `<div class="kb-card${t.done ? " done" : ""}" draggable="true" data-id="${t.id}">
...
function css() {
  ...
  .kb-del{margin-left:auto;border:0;background:transparent;color:${C.muted};cursor:pointer;font-size:14px}
  .kb-del:hover{color:${C.over}}
