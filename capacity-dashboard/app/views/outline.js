...
export async function render(root) {
  ...
  root.querySelectorAll(".ol-tw").forEach((tw) => {
    tw.onclick = () => { const id = +tw.dataset.id; collapsed.has(id) ? collapsed.delete(id) : collapsed.add(id); render(root); };
  });
}
...
function rowHtml(node, depth, projects, counts) {
  ...
  return `<div class="ol-row" style="padding-left:${12 + depth * 22}px">
    ${tw}
...
function css() {
  ...
  .ol-row{display:flex;align-items:center;gap:8px;padding-top:7px;padding-bottom:7px;padding-right:14px;border-bottom:1px solid ${C.line};font-size:13.5px}
  .ol-row:last-child{border-bottom:0}
  .ol-row:hover{background:#f7fbff}
  .ol-tw{width:16px;flex:none;text-align:center;color:${C.muted};cursor:pointer;font-size:10px;user-select:none}
  .ol-tw.none{cursor:default}
