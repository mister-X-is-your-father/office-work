...
## 8. ADR（すべて確定・[01-decisions](01-decisions.md)）

| ADR | 論点 | 状態 |
|---|---|---|
| **ADR-007 昇格モデル** | ADR-002「forkしない」を「forkを孵化器に→昇格」へ更新 | ✅ 正式化済 |
| **ADR-008 書き込み安全化(#1)** | 部分更新で関連が消える根治 | ✅ 実装済（fork nilガード） |
| **ADR-009 帰属(#3)** | 「誰の予定/実績か」 | ✅ 実装済（`user_id`=対象者・`created_by`=記録者）。SPA配線は次パス |
| **ADR-010 多担当=全員フル/会議=タスク** | 配分と会議の表現 | ✅ 実装済（按分全廃。会議は多担当タスクで自動計上） |

---

## 9. 次アクション（基盤固めフェーズ完了）

**基盤固め（非UI・DB規範まで徹底）は #1-#4,#7,#9 で完了。** 残り：
1. **P2 清掃（軽量）**：#5 members×projectusers 統合 / #6 `est:Nh` ラベル掃除 / #8 日別バッチ取得（N+1解消）。
2. **SPA 配線パス**：#3 の対象者選択UI（planner フォーム・logTime/logPlan の user_id 送信）、ガント作り込み（窓送り・締切ダイヤ・ドラッグ編集＝`updateTask`使用）、会議運用（`会議`ラベル＋表示）。
3. **残ビュー**：かんばん(59)/一覧(60)/設定(17)。設定が入ると 8h/対象PJ のハードコードが解ける。
