...
正直な体感: 準備75 / 実行80 / 運用70。キャパ保護＝第一弾＋F1＋F4(今週予算)完成。残り＝**F5 割り込みゼロサム**／**F6 ディープワーク枠(要設計判断)**。Slack無人送信は不要指示。
...
- **波F キャパ保護 残り**:
  ...
  - **F5 割り込みゼロサム（次）**: 満杯の日に新規/割り込みが来たら「何か外すか別日へ」と迫る。実装案＝taskform で期日を
    満杯日(その担当のloadByMember[day]>=capH)に設定したら inline 警告。taskform は共有＝指示役が直接編集（要 taskform 精読）。
  - **F6 ディープワーク枠（要設計判断）**: 最重要タスク専用の確保枠（保護枠の逆）。保護時間帯は backcast が「避ける」枠だが、
