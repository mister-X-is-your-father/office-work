...
正直な体感: 準備75 / 実行80 / 運用65。残り＝**波F**（F4予算/F5ゼロサム/F6ディープワーク枠）。Slack無人送信は不要指示。
...
- **波F キャパ保護 残り（次に着手）**: F4 キャパ予算可視化（週/日の残容量＝時間家計簿）/ F5 割り込みゼロサム（満杯の日に新規は「何か外すか別日へ」）/ F6 ディープワーク枠（最重要専用の確保枠＝保護枠の逆）。
