...
**実行ループ＝E1（今日やる1手）＋E2（朝の手順通知）＋E3（今すぐ着手→進行中＋ポモ）＋手順done消し込み＋E4検知（期日超過の督促）まで完成。残り＝E4自動再逆算・E5報告。**
正直な体感: 準備75 / 実行65 / 運用40。「計画を絶対に実現」の残り＝E4再逆算とE5報告（＋波F）。
...
- **波E 実行ループ（残り）**:
  - ~~E1 / E2 / E3 / 手順done化 / E4検知~~ → 完了。
  - **E4 自動再逆算（次の推奨）**: 期日超過の未完了手順を検知したら、残り未完了手順を「今日から」再逆算（backcast 既に今日床止め済）。
    実装メモ＝backcast に `!done` の手順だけ渡し、結果を元 index に戻す（done を巻き込まない）。schedule プラグイン（exec-support.js・委譲）。
    自動発火タイミング（パネル表示時／日次／手動ボタン）は要小判断＝まずは手動ボタン「遅れを今日から組み直す」が安全。
  - E5 報告・アカウンタビリティ（report/status へ進捗反映）。※上司対策C(Slack)は**ユーザー指示でいったん不要**。
- **波F キャパ保護 残り**: F4 キャパ予算可視化 / F5 割り込みゼロサム / F6 ディープワーク枠。
