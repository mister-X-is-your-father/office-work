...
## 0. 一言で
タスクの「着手の不確実性」を機械的に削る仕組み。**準備（フェーズ1）＝完成**。**キャパ保護＝第一弾完成**。
**実行ループ（今日の手順→通知→今すぐ着手→未実行検知→報告）＝未着手＝次の本丸**。
正直な体感: 準備75 / 実行25 / 運用35。「計画を絶対に実現」には実行ループが要る。
...
推奨初手から：
- **波E 実行ループ（本丸）**:
  - E1 今日の手順 自動抽出（steps.due==today を「今日やる1手」に）… **home/today＝ユーザー領域・要調整**
  - E2 着手日・トリガー通知（`lib/notify.js` 連携）… 指示役管轄寄り
  - E3 今すぐ着手→ステータス進行中＋ポモ開始（`pomodoro.js`）… 指示役管轄寄り
  - E4 未実行検知→自動再逆算 / E5 報告・アカウンタビリティ（report/status・上司対策C=要bot/webhook）
- **波F キャパ保護 残り**: F1 全タスク横断逆算（他タスクのplans日次負荷も差し引く）/ F4 キャパ予算可視化 / F5 割り込みゼロサム / F6 ディープワーク枠。
